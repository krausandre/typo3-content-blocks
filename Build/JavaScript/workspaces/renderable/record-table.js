...
let RecordTableElement = class RecordTableElement extends LitElement {
    constructor() {
        super(...arguments);
        this.results = [];
      ...
      <div class="table-fit mb-0">
        <table class="table table-striped">
          ...
          <tbody data-multi-record-selection-row-selection="true">
            ${repeat(this.results, (result) => result.uid, (data) => this.renderTableRow(data))}
          </tbody>
        </table>
      </div>
    `;
    ...
    renderTableRow(data) {
        ...
        let wsStateActionClass;
        let wsStateActionLabel;
        switch (wsState) {
            case 'deleted':
    ...
    renderElementActions(data) {
        return [
            this.getAction(data.allowedAction_view, 'preview', 'actions-version-workspace-preview', {
                'title': TYPO3.lang['tooltip.viewElementAction']
            }),
            this.getAction(data.allowedAction_edit && data.state_Workspace !== 'deleted', 'open', 'actions-open', {
                'title': TYPO3.lang['tooltip.editElementAction']
            }),
            this.getAction(data.allowedAction_versionPageOpen, 'version', 'actions-version-page-open', {
                'title': TYPO3.lang['tooltip.openPage']
            })
        ];
    }
    renderVersioningActions(data) {
        const hasSubitems = data.Workspaces_CollectionChildren > 0 && data.Workspaces_CollectionCurrent !== '';
        return [
            this.getAction(hasSubitems, 'expand', (data.expanded ? 'actions-caret-down' : 'actions-caret-right'), {
            ...
            this.getAction(data.hasChanges, 'changes', 'actions-document-info', {
                'title': TYPO3.lang['tooltip.showChanges']
            }),
            this.getAction(data.allowedAction_publish && data.Workspaces_CollectionParent === '', 'publish', 'actions-version-swap-version', {
                'title': TYPO3.lang['tooltip.publish']
            }),
            this.getAction(data.allowedAction_delete, 'remove', 'actions-delete', {
     ...
     * @return {JQuery}
     */
    getAction(condition, action, iconIdentifier, additionalAttributes) {
        return html `
      <button
        type="button"
...
__decorate([
    property({ type: Array })
], RecordTableElement.prototype, "results", void 0);
RecordTableElement = __decorate([
    customElement('typo3-workspaces-record-table')
], RecordTableElement);
export { RecordTableElement };
