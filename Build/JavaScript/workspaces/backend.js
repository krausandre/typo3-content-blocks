...
(function (Identifiers) {
    ...
    Identifiers["stagesSelector"] = "#workspace-settings-form select[name=\"stages\"]";
    ...
    Identifiers["publishAction"] = "[data-action=\"publish\"]";
    Identifiers["prevStageAction"] = "[data-action=\"prevstage\"]";
    Identifiers["nextStageAction"] = "[data-action=\"nextstage\"]";
    Identifiers["changesAction"] = "[data-action=\"changes\"]";
    Identifiers["previewAction"] = "[data-action=\"preview\"]";
    Identifiers["openAction"] = "[data-action=\"open\"]";
    Identifiers["versionAction"] = "[data-action=\"version\"]";
    Identifiers["removeAction"] = "[data-action=\"remove\"]";
    Identifiers["expandAction"] = "[data-action=\"expand\"]";
    Identifiers["workspaceRecipientsSelectAll"] = ".t3js-workspace-recipients-selectall";
    Identifiers["workspaceRecipientsDeselectAll"] = ".t3js-workspace-recipients-deselectall";
...
class Backend extends Workspaces {
    constructor() {
        ...
        this.handleCheckboxStateChanged = (event) => {
            ...
            const chooseMassAction = document.querySelector(Identifiers.chooseMassAction);
            if (chooseMassAction !== null) {
                chooseMassAction.disabled = this.markedRecordsForMassAction.length > 0;
            }
        };
        this.openIntegrityWarningModal = () => {
            const modal = Modal.confirm(TYPO3.lang['window.integrity_warning.title'], html `<p>${TYPO3.lang['integrity.hasIssuesDescription']}<br>${TYPO3.lang['integrity.hasIssuesQuestion']}</p>`, SeverityEnum.warning);
        ...
        DocumentService.ready().then(() => {
            ...
            this.settings.depth = document.querySelector(Identifiers.depthSelector)?.value;
            this.settings.language = document.querySelector(Identifiers.languageSelector)?.value;
            this.settings.stage = document.querySelector(Identifiers.stagesSelector)?.value;
     ...
     */
    static changeCollectionParentState(collection, check) {
        const parent = document.querySelector('tr[data-collection-current="' + collection + '"] input[type=checkbox]');
     ...
     * module from overriding the manually changed state.
     */
    static changeCollectionChildrenState(collectionCurrent, check) {
        const collectionChildren = document.querySelectorAll(selector `tr[data-collection="${collectionCurrent}"] input[type=checkbox]`);
        if (collectionChildren.length) {
            collectionChildren.forEach((checkbox) => {
    ...
    registerEvents() {
        new RegularEvent('click', (event, target) => {
            ...
            this.checkIntegrity({
                ...
                if ((await response.resolve())[0].result.result === 'warning') {
                    this.openIntegrityWarningModal().addEventListener('confirm.button.ok', () => {
        ...
        new RegularEvent('submit', (event) => {
            ...
            const searchTextField = document.querySelector(Identifiers.searchTextField);
        ...
        new RegularEvent('change', (event, target) => {
            ...
            this.sendRemoteRequest(this.generateRemotePayload('getWorkspaceInfos', this.settings)).then(async (response) => {
                ...
                target.previousElementSibling.innerHTML = target.querySelector('option:checked').dataset.icon;
    ...
    renderWorkspaceInfos(result) {
        const contentsContainer = document.querySelector(Identifiers.contentsContainer);
        const noContentsContainer = document.querySelector(Identifiers.noContentsContainer);
    ...
    buildPagination(totalItems) {
        const paginationContainer = document.querySelector(Identifiers.pagination);
        if (totalItems === 0) {
        ...
        let pagination = paginationContainer.querySelector('typo3-backend-pagination');
        if (pagination === null) {
    ...
    viewChanges(event, target) {
        ...
        this.sendRemoteRequest(this.generateRemotePayload('getRowDetails', {
            ...
            Modal.advanced({
                ...
                title: TYPO3.lang['window.recordInformation'].replace('{0}', tableRow.querySelector('.t3js-title-workspace').innerText.trim()),
                content: content,
     ...
     */
    confirmDeleteRecordFromWorkspace(e, target) {
        const tableRow = target.closest('tr');
     ...
     * Runs a mass action
     */
    runSelectionAction(event, target) {
            ...
            buttons: [
                {
                    ...
                    action: new DeferredAction(async () => {
                        await this.sendRemoteRequest(this.generateRemoteActionsPayload('publishSingleRecord', [
                            row.dataset.table,
    ...
    renderSelectionActionModal(selectedAction, affectedRecords) {
        const modal = Modal.advanced({
            ...
            buttons: [
                {
                    ...
                    btnClass: 'btn-warning',
                    action: new DeferredAction(async () => {
                        await this.sendRemoteRequest(this.generateRemoteActionsPayload('executeSelectionAction', {
        ...
        modal.addEventListener('typo3-modal-hidden', () => {
            const chooseSelectionAction = document.querySelector(Identifiers.chooseSelectionAction);
    ...
    renderMassActionModal(selectedAction) {
        let massAction;
            ...
            buttons: [
                {
                    ...
                    btnClass: 'btn-warning',
                    action: new DeferredAction(async () => {
                        const response = await this.sendRemoteRequest(this.generateRemoteMassActionsPayload(massAction, {
                            init: true,
                            total: 0,
                            processed: 0,
                            language: this.settings.language
     ...
     */
    sendToSpecificStageAction(event, target) {
        const affectedRecords = [];
        const stage = target.value;
        for (let i = 0; i < this.markedRecordsForMassAction.length; ++i) {
            const affected = this.markedRecordsForMassAction[i].split(':');
        ...
        this.sendRemoteRequest(this.generateRemoteActionsPayload('sendToSpecificStageWindow', [stage])).then(async (response) => {
            const modal = this.renderSendToStageWindow(await response.resolve());
            modal.addEventListener('button.clicked', (modalEvent) => {
                const target = modalEvent.target;
                if (target.name === 'ok') {
            ...
            modal.addEventListener('typo3-modal-hide', () => {
                const chooseStageAction = document.querySelector(Identifiers.chooseStageAction);
    ...
    resetMassActionState(hasRecords) {
        this.markedRecordsForMassAction = [];
        if (hasRecords) {
            const workspaceActions = document.querySelector(Identifiers.workspaceActions);
