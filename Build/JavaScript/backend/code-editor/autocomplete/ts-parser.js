...
export class TreeNode {
     ...
     */
    getNodeTypeFromTsref() {
        const path = this.extPath.split('.'), lastSeg = path.pop();
...
class Stack extends Array {
    lastElementEquals(el) {
        return this.length > 0 && this[this.length - 1] === el;
    }
    popIfLastElementEquals(el) {
...
export class TsParser {
    constructor(tsRef, extTsObjTree) {
        this.tsRef = tsRef;
     ...
     */
    buildTsObjTree(completionState) {
        this.tsTree = new TreeNode('', this);
        this.tsTree.value = 'TLO';
        let currentLine = 1, line = '', ignoreLine = false, insideCondition = false;
    ...
    getTreeNode(path) {
        ...
        let subTree = this.tsTree.childNodes, pathSeg, parent = this.tsTree;
     ...
     */
    setReference(path1, path2) {
        const path1arr = path1.split('.'), lastNodeName = path1arr[path1arr.length - 1], treeNode1 = this.getTreeNode(path1), treeNode2 = this.getTreeNode(path2);
