...
class DateTimePicker {
    constructor() {
        this.format = (typeof opener?.top?.TYPO3 !== 'undefined' ? opener.top : top).TYPO3.settings.DateTimePicker.DateFormat;
    }
    /**
    ...
    initialize(element) {
        ...
        import('flatpickr/dist/l10n').then(() => {
    ...
    initializeField(inputElement, locale) {
        ...
        if (dateTimePicker.altInput instanceof HTMLInputElement) {
            ...
            dateTimePicker.input.addEventListener('typo3:internal:clear', () => {
                // @todo The `typo3:internal:clear` event is a hack and must fall again
                dateTimePicker.clear();
            });
        }
        dateTimePicker._input.addEventListener('change', (e) => {
            // Update selected date in picker after manually entering a value
            const value = e.target.value;
            const parsedDate = dateTimePicker.parseDate(value, dateTimePicker.config.altFormat);
            dateTimePicker.setDate(parsedDate, true);
        });
        dateTimePicker._input.addEventListener('keyup', (e) => {
            if (e.key === 'Escape') {
    ...
    getDateOptions(inputElement) {
        ...
        const options = {
            ...
            parseDate: (currentDateString, format) => {
                if (format === ISO8601_UTC) {
            ...
            onReady: (dates, currentDateString, self) => {
                if (self.altInput !== undefined) {
                    ...
                    self.altInput.form.addEventListener('t3-formengine-postfieldvalidation', (e) => {
            ...
            onChange: (dates, currentDateString, self) => {
