...
export default class FormEngineValidation {
    static { this.rulesSelector = '[data-formengine-validation-rules]'; }
    static { this.inputSelector = '[data-formengine-input-params]'; }
    static { this.markerSelector = '.t3js-formengine-validation-marker'; }
    static { this.labelSelector = '.t3js-formengine-label'; }
    static { this.errorClass = 'has-error'; }
    static { this.validationErrorClass = 'has-validation-error'; }
    static { this.passwordDummy = '********'; }
    /**
     * Initialize validation for the first time
     */
    static initialize(formEngine) {
        formEngineInstance = formEngine;
        formEngineInstance.formElement.querySelectorAll('.' + FormEngineValidation.errorClass).forEach((e) => e.classList.remove(FormEngineValidation.errorClass));
     ...
     */
    static initializeInputFields() {
        formEngineInstance.formElement.querySelectorAll(FormEngineValidation.inputSelector).forEach((visibleField) => {
            ...
            const actualValueField = formEngineInstance.formElement.querySelector(selector `[name="${fieldName}"]`);
     ...
     */
    static initializeInputField(fieldName) {
        const field = formEngineInstance.formElement.querySelector(selector `[name="${fieldName}"]`);
        const humanReadableField = formEngineInstance.formElement.querySelector(selector `[data-formengine-input-name="${fieldName}"]`);
    ...
    static registerCustomEvaluation(name, handler) {
        if (!customEvaluations.has(name)) {
            customEvaluations.set(name, handler);
        }
    }
    static formatByEvals(config, value) {
     ...
     */
    static updateInputField(fieldName) {
        const field = formEngineInstance.formElement.querySelector(selector `[name="${fieldName}"]`);
        const humanReadableField = formEngineInstance.formElement.querySelector(selector `[data-formengine-input-name="${fieldName}"]`);
        if (field.dataset.config !== undefined) {
            const config = JSON.parse(field.dataset.config);
    ...
    static validateField(field, value) {
        ...
        const rules = JSON.parse(field.dataset.formengineValidationRules);
                ...
                case 'range':
                    if (value !== '') {
                        if (rule.minItems || rule.maxItems) {
                            relatedField = formEngineInstance.formElement.querySelector(selector `[name="${field.dataset.relatedfieldname}"]`);
        ...
        formEngineInstance.formElement.dispatchEvent(new CustomEvent('t3-formengine-postfieldvalidation', { detail: { field: field, isValid: isValid }, cancelable: false, bubbles: true }));
        return returnValue;
    }
    static processByEvals(config, value) {
        if (config.evalList !== undefined) {
            const evalList = Utility.trimExplode(',', config.evalList);
            for (const evalInstruction of evalList) {
     ...
     */
    static processValue(command, value, config) {
        let newString = '';
        let theValue = '';
        let a = 0;
    ...
    static validate(section) {
        if (typeof section === 'undefined' || section instanceof Document) {
            formEngineInstance.formElement.querySelectorAll(FormEngineValidation.markerSelector + ', .t3js-tabmenu-item').forEach((tabMenuItem) => {
     ...
     */
    static markFieldAsChanged(field) {
        if (field instanceof $) {
            // @deprecated
     ...
     */
    static parseDouble(value, precision = 2) {
        let theVal = '' + value;
     ...
     */
    static markParentTab(element, isValid) {
    ...
    static showErrorModal() {
        const modal = Modal.confirm(TYPO3.lang.alert || 'Alert', TYPO3.lang['FormEngine.fieldsMissing'], Severity.error, [
            {
