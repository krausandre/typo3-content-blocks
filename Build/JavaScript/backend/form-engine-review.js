...
export class FormEngineReview {
     ...
     */
    initialize() {
        this.formElement.addEventListener('t3-formengine-postfieldvalidation', (e) => {
     ...
     */
    attachButtonToModuleHeader() {
        const leastButtonBar = document.querySelector('.t3js-module-docheader-bar-buttons').lastElementChild.querySelector('[role="toolbar"]');
    ...
    checkForReviewableField() {
        const toggleButton = document.querySelector('.' + this.toggleButtonClass);
        ...
        if (this.invalidFields.size > 0) {
            ...
            for (const invalidField of this.invalidFields) {
                ...
                const relatedInputField = fieldContainer.querySelector('[data-formengine-validation-rules]');
            ...
            Popover.setOptions(toggleButton, {
                html: true,
                content: erroneousListGroup
            });
        }
        else {
     ...
     */
    switchToField(e, fieldContainer, inputField) {
        e.preventDefault();
