...
import { TreeNodeCommandEnum, TreeNodePositionEnum } from '@typo3/backend/tree/tree-node';
...
let EditablePageTree = class EditablePageTree extends PageTree {
    ...
    handleNodeDelete(node) {
        ...
        if (this.settings.displayDeleteConfirmation) {
            const modal = Modal.confirm(TYPO3.lang['mess.delete.title'], TYPO3.lang['mess.delete'].replace('%s', options.node.name), Severity.warning, [
                {
    ...
    handleNodeMove(node, target, position) {
        ...
        const modal = Modal.confirm(TYPO3.lang.move_page, modalText, Severity.warning, [
            {
                text: TYPO3.lang['labels.cancel'] || 'Cancel',
                active: true,
                btnClass: 'btn-default',
...
let PageTreeNavigationComponent = class PageTreeNavigationComponent extends TreeModuleState(LitElement) {
    constructor() {
        super(...arguments);
        ...
        this.showContextMenu = (evt) => {
            ...
            ContextMenu.show(node.recordType, node.identifier, 'tree', '', '', this.tree.getElementFromNode(node), evt.detail.originalEvent);
        };
    }
    connectedCallback() {
...
__decorate([
    query('typo3-backend-navigation-component-pagetree-toolbar')
...
let PageTreeToolbar = class PageTreeToolbar extends TreeToolbar {
    constructor() {
        super(...arguments);
        this.tree = null;
    }
      ...
      <div class="tree-toolbar">
        ...
        <div class="tree-toolbar__submenu">
          ${this.tree?.settings?.doktypes?.length
            ? this.tree.settings.doktypes.map((item) => {
                return html `
                <div
