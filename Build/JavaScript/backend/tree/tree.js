...
import { TreeNodeCommandEnum, TreeNodePositionEnum } from './tree-node';
...
export class Tree extends LitElement {
    constructor() {
        ...
        this.openNodeTimeout = { targetNode: null, timeout: null };
        this.unfilteredNodes = '';
        this.muteErrorNotifications = false;
    ...
    async fetchData(parentNode = null) {
        try {
            ...
            const all = await Promise.all(nodes.map(async (node) => {
    ...
    async editNode(node) {
        if (this.isNodeEditable(node)) {
            ...
            this.requestUpdate();
            this.updateComplete.then(() => {
                const inputField = this.getElementFromNode(this.editingNode)?.querySelector('.node-edit');
                if (inputField) {
                    inputField.focus();
                    inputField.select();
                }
    ...
    async addNode(node, target, position) {
    ...
    getNodeByTreeIdentifier(treeIdentifier) {
        return this.nodes.find((node) => {
            return node.__treeIdentifier === treeIdentifier;
        });
    }
    getNodeDragStatusIcon() {
    ...
    render() {
      ...
      <div class="nodes-container">
        ...
        <div
          class="nodes-root"
          @scroll="${(event) => { this.currentScrollPosition = event.currentTarget.scrollTop; }}"
          @mouseover=${() => this.isOverRoot = true}
          @mouseout=${() => this.isOverRoot = false}
          @keydown=${(event) => this.handleKeyboardInteraction(event)}
        >
    ...
    renderVisibleNodes() {
        ...
        this.displayNodes = this.nodes.filter((node) => {
            return node.__hidden !== true && !node.__treeParents.some((parentTreeIdentifier) => Boolean(blacklist.indexOf(parentTreeIdentifier) !== -1));
        });
      ...
      <div class="nodes-list" role="tree" style="${styleMap({ 'height': (this.displayNodes.length * this.nodeHeight) + 'px' })}">
        ${repeat(visibleNodes, (node) => this.getNodeTreeIdentifier(node), (node) => html `
          <div
            ...
            @dragover="${(event) => { this.handleNodeDragOver(event); }}"
            @dragstart="${(event) => { this.handleNodeDragStart(event, node); }}"
            @dragleave="${(event) => { this.handleNodeDragLeave(event); }}"
            @dragend="${(event) => { this.handleNodeDragEnd(event); }}"
            @drop="${(event) => { this.handleNodeDrop(event); }}"

            @click="${(event) => { this.handleNodeClick(event, node); }}"
            @dblclick="${(event) => { this.handleNodeDoubleClick(event, node); }}"
            @focusin="${() => { this.focusedNode = node; }}"
            @focusout="${() => { if (this.focusedNode === node) {
            this.lastFocusedNode = node;
            this.focusedNode = null;
        } }}"
            @contextmenu="${(event) => { event.preventDefault(); event.stopPropagation(); this.dispatchEvent(new CustomEvent('typo3:tree:node-context', { detail: { node, originalEvent: event } })); }}"
          >
    ...
    handleNodeDragStart(event, node) {
        ...
        this.createDataTransferItemsFromNode(node).forEach(({ data, type }) => event.dataTransfer.items.add(data, type));
    ...
    createNodeToggle(node) {
        ...
        return node.hasChildren === true
            ? html `
          <span class="node-toggle" @click="${(event) => { event.preventDefault(); event.stopImmediatePropagation(); this.handleNodeToggle(node); }}">
            <typo3-backend-icon
    ...
    createNodeContentIcon(node) {
        return this.settings.showIcons
            ? html `
        <span class="node-icon"
          @click="${(event) => { event.preventDefault(); event.stopImmediatePropagation(); this.dispatchEvent(new CustomEvent('typo3:tree:node-context', { detail: { node: node, originalEvent: event } })); }}"
          @dblclick="${(event) => { event.preventDefault(); event.stopImmediatePropagation(); }}"
        >
          <typo3-backend-icon
    ...
    createNodeStatusInformation(node) {
    ...
    createNodeForm(node) {
        ...
        const keydownFunction = (event) => {
            ...
            if ([KeyTypes.ENTER, KeyTypes.TAB].includes(code)) {
                const target = event.target;
            ...
            else if ([KeyTypes.ESCAPE].includes(code)) {
                this.editingNode = null;
                this.requestUpdate();
    ...
    getNodeTreeIdentifier(node) {
        return node.__treeIdentifier;
    }
    getNodeParentTreeIdentifier(node) {
     ...
     */
    scrollNodeIntoVisibleArea(node) {
    ...
    registerUnloadHandler() {
        try {
            ...
            window.addEventListener('pagehide', () => this.muteErrorNotifications = true, { once: true });
        }
        catch {
...
], Tree.prototype, "setup", void 0);
__decorate([
    state()
], Tree.prototype, "settings", void 0);
__decorate([
    query('.nodes-root')
], Tree.prototype, "root", void 0);
__decorate([
    state()
], Tree.prototype, "nodes", void 0);
__decorate([
    state()
], Tree.prototype, "currentScrollPosition", void 0);
__decorate([
    state()
], Tree.prototype, "currentVisibleHeight", void 0);
__decorate([
    state()
], Tree.prototype, "searchTerm", void 0);
__decorate([
    state()
], Tree.prototype, "loading", void 0);
__decorate([
    state()
], Tree.prototype, "hoveredNode", void 0);
__decorate([
    state()
], Tree.prototype, "nodeDragAllowed", void 0);
