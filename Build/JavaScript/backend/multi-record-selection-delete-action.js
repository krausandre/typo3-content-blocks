                    ...
                    trigger: async (e, modal) => {
                        ...
                        try {
                            const result = await AjaxDataHandler.process({
                                cmd: {
                                    [tableName]: Object.fromEntries(entityIdentifiers.map((identifier) => [
                                        identifier, { delete: 1 }
                                    ]))
                                }
                            });
                            if (result.hasErrors) {
                                ...
                                event.target.ownerDocument.location.href = returnUrl;
                            }
                            else {
                                event.target.ownerDocument.location.reload();
                            }
                        }
                        catch {
