...
class PageActions {
    constructor() {
        DocumentService.ready().then(() => {
            const pageLayoutToggleShowHidden = document.getElementById('pageLayoutToggleShowHidden');
    ...
    toggleContentElementVisibility(e) {
        const me = e.target;
        const hiddenElements = document.querySelectorAll(IdentifierEnum.hiddenElements);
        ...
        PersistentStorage.set('moduleData.web_layout.showHidden', show ? '1' : '0').then(() => {
            me.disabled = false;
        });
    }
}
