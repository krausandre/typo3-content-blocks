...
class Localization {
    ...
    async initialize() {
        ...
        $(document).on('click', this.triggerButton, async (e) => {
            e.preventDefault();
            ...
            if ($triggerButton.data('allowTranslate') === 0 && $triggerButton.data('allowCopy') === 0) {
                Modal.confirm(TYPO3.lang['window.localization.mixed_mode.title'], TYPO3.lang['window.localization.mixed_mode.message'], SeverityEnum.warning, [
                    {
            ...
            const availableLanguages = await (await this.loadAvailableLanguages(parseInt($triggerButton.data('pageId'), 10), parseInt($triggerButton.data('languageId'), 10))).resolve();
            ...
            if (availableLocalizationModes.length === 1) {
                ...
                MultiStepWizard.addSlide('localize-choose-action', TYPO3.lang['localize.wizard.header_page']
                    .replace('{0}', $triggerButton.data('page'))
                    .replace('{1}', $triggerButton.data('languageName')), buttonContainer, SeverityEnum.notice, TYPO3.lang['localize.wizard.step.selectMode'], ($slide, settings) => {
            ...
            if (availableLanguages.length === 1) {
                ...
                MultiStepWizard.addSlide('localize-choose-language', TYPO3.lang['localize.view.chooseLanguage'], '', SeverityEnum.notice, TYPO3.lang['localize.wizard.step.chooseLanguage'], async ($slide, settings) => {
            ...
            MultiStepWizard.addSlide('localize-summary', TYPO3.lang['localize.view.summary'], '', SeverityEnum.notice, TYPO3.lang['localize.wizard.step.selectRecords'], async ($slide, settings) => {
                $slide.empty().html('<div class="text-center">' + (await Icons.getIcon('spinner-circle', Icons.sizes.large)) + '</div>');
                const result = await (await this.getSummary(parseInt($triggerButton.data('pageId'), 10), parseInt($triggerButton.data('languageId'), 10), settings.sourceLanguage)).resolve();
                $slide.empty();
            ...
            MultiStepWizard.addFinalProcessingSlide(async ($slide, settings) => {
                await this.localizeRecords(parseInt($triggerButton.data('pageId'), 10), parseInt($triggerButton.data('languageId'), 10), settings.sourceLanguage, settings.localizationMode, settings.records);
     ...
     * Localize records
     */
    localizeRecords(pageId, languageId, sourceLanguage, localizationMode, uidList) {
