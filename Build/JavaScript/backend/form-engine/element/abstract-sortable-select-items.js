...
export class AbstractSortableSelectItems {
    constructor() {
        /**
        ...
        this.registerSortableEventHandler = (fieldElement) => {
            ...
            aside.addEventListener('click', (e) => {
                const target = e.target.closest('.t3js-btn-option');
                if (target === null) {
                    return;
                }
                e.preventDefault();
    ...
    static removeOption(fieldElement, availableFieldElement) {
        ...
        fieldElement.querySelectorAll(':checked').forEach((option) => {
            const originalOption = availableFieldElement.querySelector(selector `option[value="${option.value}"]`);
