...
class ColorElement extends HTMLElement {
    ...
    registerEventHandler() {
        const hiddenElement = document.querySelector(selector `input[name="${this.element.dataset.formengineInputName}"]`);
        new RegularEvent('blur', (e) => {
            hiddenElement.value = e.target.value;
            this.handleEvent(e);
        }).bindTo(this.element);
        new RegularEvent('formengine.cp.change', (e) => {
            this.handleEvent(e);
        }).bindTo(this.element);
    }
    handleEvent(e) {
