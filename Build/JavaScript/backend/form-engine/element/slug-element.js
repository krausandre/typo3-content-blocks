...
class SlugElement {
    ...
    registerEvents() {
        const fieldsToListenOnList = Object.values(this.getAvailableFieldsForProposalGeneration()).map((field) => `[id="${field.id}"]`);
    ...
    getAvailableFieldsForProposalGeneration() {
        ...
        for (const [fieldName, selector] of Object.entries(this.fieldsToListenOn)) {
            let field = document.querySelector('[data-formengine-input-name="' + selector + '"]');
            if (field === null) {
