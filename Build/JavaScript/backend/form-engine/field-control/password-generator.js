...
class PasswordGenerator {
    constructor(controlElementId) {
        ...
        this.humanReadableField = null;
        this.hiddenField = null;
        this.passwordRules = null;
        DocumentService.ready().then(() => {
            this.controlElement = document.getElementById(controlElementId);
            this.humanReadableField = document.querySelector('input[data-formengine-input-name="' + this.controlElement.dataset.itemName + '"]');
            this.hiddenField = document.querySelector('input[name="' + this.controlElement.dataset.itemName + '"]');
            ...
            if (!this.controlElement.dataset.allowEdit) {
                ...
                if (this.humanReadableField.isClearable || this.humanReadableField.classList.contains('t3js-clearable')) {
                    ...
                    const clearableContainer = this.humanReadableField.closest('div.form-control-clearable-wrapper');
                    if (clearableContainer) {
                        clearableContainer.classList.remove('form-control-clearable');
                        const closeButton = clearableContainer.querySelector('button.close');
                        if (closeButton) {
