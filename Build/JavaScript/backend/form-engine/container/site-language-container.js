...
(function (Selectors) {
    ...
    Selectors["createNewRecordPresetSelector"] = ".t3js-create-new-preset";
})(Selectors || (Selectors = {}));
var States;
(function (States) {
...
class SiteLanguageContainer extends HTMLElement {
    ...
    static collapseExpandRecord(objectId) {
        ...
        const collapseButton = document.querySelector(selector `[aria-controls="${objectId}_fields"]`);
    ...
    async connectedCallback() {
        ...
        this.container = this.querySelector(selector `#${identifier}`);
    ...
    registerCreateRecordByPresetSelector() {
        new RegularEvent('change', (e, targetElement) => {
            e.preventDefault();
            e.stopImmediatePropagation();
            const selector = this.container.querySelector(Selectors.createNewRecordPresetSelector);
