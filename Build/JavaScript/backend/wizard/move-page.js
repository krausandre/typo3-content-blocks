...
export class MovePage {
    ...
    registerEvents(container) {
        ...
        new RegularEvent('click', async (e, actionElement) => {
            const isCopyAction = document.querySelector('#makeCopy').checked;
            ...
            AjaxDataHandler.process(parameters).then(() => {
                Modal.dismiss();
                Notification.success(lll(isCopyAction ? 'movePage.notification.pageCopied.title' : 'movePage.notification.pageMoved.title'), lll(isCopyAction ? 'movePage.notification.pageCopied.message' : 'movePage.notification.pageMoved.message', recordTitle), 10, [
                    {
