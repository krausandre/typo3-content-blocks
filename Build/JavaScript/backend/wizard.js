...
class Wizard {
    constructor() {
        console.warn('The module `@typo3/backend/wizard.js` has been marked as deprecated and will be removed in TYPO3 v14.0. '
    ...
    addSlide(identifier, title, content = '', severity = SeverityEnum.notice, callback) {
        const slide = {
    ...
    addFinalProcessingSlide(callback) {
        ...
        return Icons.getIcon('spinner-circle', Icons.sizes.large, null, null).then((markup) => {
            const $processingSlide = $('<div />', { class: 'text-center' }).append(markup);
            this.addSlide('final-processing-slide', top.TYPO3.lang['wizard.processing.title'], $processingSlide[0].outerHTML, Severity.notice, callback);
        });
    }
    show() {
        ...
        Modal.advanced({
            ...
            callback: (modal) => {
                topLevelModuleImport('@typo3/backend/element/progress-bar-element.js').then(() => {
    ...
    initializeEvents(modal) {
        ...
        this.setup.$carousel.get(0).addEventListener('slide.bs.carousel', () => {
