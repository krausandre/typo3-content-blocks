...
class PageTsConfigIncludes {
    ...
    async registerEventListeners() {
        ...
        document.querySelectorAll('.t3js-pagetsconfig-includes-modal').forEach((link) => {
            link.addEventListener('click', (e) => {
                ...
                const content = html `${until(this.fetchModalContent(url), html `<div class="modal-loading"><typo3-backend-spinner size="large"></typo3-backend-spinner></div>`)}`;
    ...
    async fetchModalContent(url) {
        ...
        const source = await response.resolve();
        const mode = {
