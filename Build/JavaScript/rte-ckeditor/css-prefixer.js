...
export function prefixAndRebaseCss(contents, currentPath, prefix) {
...
export function cssRelocator(currentPath) {
    return (node) => {
        if (node.type !== 'Url') {
            return;
        }
...
export function cssPrefixer(prefix) {
    if (prefix === '') {
        return () => {
            return;
        };
    ...
    return (node) => {
        ...
        const prefixSelector = csstree.find(prefixAst, (node) => node.type === 'Selector');
        if (prefixSelector === null) {
            throw new Error(`Failed to parse "${prefix}" as CSS prefix`);
        }
        if (selector.children.first.type === 'PseudoClassSelector' &&
