...
import { default as modalObject } from '@typo3/backend/modal';
...
export class Typo3LinkCommand extends Core.Command {
    ...
    refresh() {
        ...
        for (const attribute of this.getLinkAttributesAllowedOnText(model.schema)) {
            ...
            if (attribute === ghsAttributeName) {
                const value = sourceSelection.getAttribute(attribute);
        ...
        model.change(writer => {
            ...
            if (selection.isCollapsed) {
                ...
                if (selection.hasAttribute('linkHref')) {
                    ...
                    const linkRange = Typing.findAttributeRange(position, 'linkHref', selection.getAttribute('linkHref'), model);
    ...
    getLinkAttributesAllowedOnText(schema) {
        const textAttributes = schema.getDefinition('$text').allowAttributes;
        return textAttributes.filter(attribute => attribute.startsWith('link') || attribute === 'htmlA');
    }
    removeLinkAttributesFromSelection(writer, linkAttributes) {
        writer.removeSelectionAttribute('linkHref');
        for (const attribute of linkAttributes) {
            writer.removeSelectionAttribute(attribute);
        }
    }
    composeLinkAttributes(linkAttr) {
        const attrs = {};
        for (const [attribute, value] of Object.entries(linkAttr.attrs)) {
            if (attribute === 'linkClass') {
                ...
                if (selection.hasAttribute(ghsAttributeName)) {
                    htmlA = { ...selection.getAttribute(ghsAttributeName) };
                }
                else {
                    htmlA = {};
                }
    ...
    isRangeToUpdate(range, allowedRanges) {
    ...
    execute() {
        ...
        model.change(writer => {
            ...
            const rangesToUnlink = selection.isCollapsed
                ? [Typing.findAttributeRange(selection.getFirstPosition(), 'linkHref', selection.getAttribute('linkHref'), model)]
...
export class Typo3LinkEditing extends Core.Plugin {
    static { this.pluginName = 'Typo3LinkEditing'; }
    init() {
        ...
        const ghsDataFilter = editor.plugins.get('DataFilter');
        ...
        editor.conversion.for('upcast').elementToAttribute({
            ...
            model: {
                ...
                value: (viewElement) => viewElement.getAttribute('data-rte-error')
            }
        });
        // linkTitle <=> title
        editor.conversion.for('downcast').attributeToElement({
...
export class Typo3LinkUI extends Core.Plugin {
    static get requires() {
        return [UI.ContextualBalloon, Link.LinkEditing];
    }
    ...
    findLinkElementAncestor(position) {
        return position.getAncestors().find((ancestor) => LinkUtils.isLinkElement(ancestor));
    }
    openLinkBrowser(editor) {
        const linkCommand = editor.commands.get('link');
        ...
        this.openElementBrowser(editor, 'Link', this.makeUrlFromModulePath(editor, editor.config.get('typo3link')?.routeUrl, additionalParameters));
    }
    makeUrlFromModulePath(editor, routeUrl, parameters) {
        return routeUrl
            + (routeUrl.indexOf('?') === -1 ? '?' : '&')
    ...
    openElementBrowser(editor, title, url) {
...
export class Typo3Link extends Core.Plugin {
    static { this.overrides = [Link.Link]; }
    static get requires() {
        return ['GeneralHtmlSupport', Link.LinkEditing, Link.AutoLink, Typo3LinkEditing, Typo3LinkUI];
    }
    static get pluginName() {
        return 'Typo3Link';
    }
