...
export class Whitespace extends Core.Plugin {
    static { this.pluginName = 'Whitespace'; }
    static { this.requires = [Typing.Typing]; }
    init() {
        ...
        editor.conversion.for('editingDowncast').add(downcastDispatcher => {
            downcastDispatcher.on('insert:$text', (evt, data, conversionApi) => {
                ...
                chunks.forEach((chunk) => {
                    ...
                    viewWriter.insert(conversionApi.mapper.toViewPosition(currentPosition), viewWriter.createText(text));
                    if (chunk === '\u00AD' || chunk === '\u00A0') {
                        ...
                        const wrapperRange = viewWriter.createRange(conversionApi.mapper.toViewPosition(currentPosition), conversionApi.mapper.toViewPosition(currentPosition.getShiftedBy(chunk.length)));
