...
var DashboardWidgetMoveIntend;
(function (DashboardWidgetMoveIntend) {
    DashboardWidgetMoveIntend["start"] = "start";
    DashboardWidgetMoveIntend["end"] = "end";
    DashboardWidgetMoveIntend["left"] = "left";
    DashboardWidgetMoveIntend["right"] = "right";
    DashboardWidgetMoveIntend["up"] = "up";
    DashboardWidgetMoveIntend["down"] = "down";
})(DashboardWidgetMoveIntend || (DashboardWidgetMoveIntend = {}));
const newRecordWizardEventName = 'typo3:dashboard:widget:add';
export class DashboardWidgetContentRenderedEvent extends Event {
    static { this.eventName = 'typo3:dashboard:widget:content:rendered'; }
    constructor(widget) {
...
class DashboardWidgetMoveIntendEvent extends Event {
    static { this.eventName = 'typo3:dashboard:widget:moveIntend'; }
    constructor(identifier, intend) {
...
class DashboardWidgetRemoveEvent extends Event {
    static { this.eventName = 'typo3:dashboard:widget:remove'; }
...
class DashboardWidgetRefreshEvent extends Event {
    static { this.eventName = 'typo3:dashboard:widget:refresh'; }
    constructor(identifier) {
...
class DashboardAddEvent extends Event {
    static { this.eventName = 'typo3:dashboard:dashboard:add'; }
    constructor(preset, title) {
...
class DashboardEditEvent extends Event {
    static { this.eventName = 'typo3:dashboard:dashboard:edit'; }
...
class DashboardUpdateEvent extends Event {
    static { this.eventName = 'typo3:dashboard:dashboard:update'; }
    constructor(identifier, widgets, widgetPositions) {
...
class DashboardDeleteEvent extends Event {
    static { this.eventName = 'typo3:dashboard:dashboard:delete'; }
    constructor(identifier) {
        super(DashboardDeleteEvent.eventName);
        this.identifier = identifier;
    }
}
function createSet(item) {
    const set = new Set();
...
let Dashboard = class Dashboard extends LitElement {
    constructor() {
        super();
        this.loading = false;
        this.dashboards = [];
        ...
        this.clientStorageIdentifier = 'dashboard/current_dashboard';
        ...
        // Refresh Widget
        this.addEventListener(DashboardWidgetRefreshEvent.eventName, (event) => {
        ...
        // Remove Widget
        this.addEventListener(DashboardWidgetRemoveEvent.eventName, (event) => {
            event.preventDefault();
        ...
        // Try to move widget
        this.addEventListener(DashboardWidgetMoveIntendEvent.eventName, (event) => {
            event.preventDefault();
            ...
            switch (intend) {
                ...
                case DashboardWidgetMoveIntend.end:
                    ...
                    this.dispatchEvent(new DashboardUpdateEvent(this.currentDashboard.identifier, this.currentDashboard.widgets, this.currentDashboard.widgetPositions));
                    return;
                default:
                    return;
            }
        ...
        // Add dashboard
        this.addEventListener(DashboardAddEvent.eventName, (event) => {
            event.preventDefault();
        ...
        // Edit dashboard
        this.addEventListener(DashboardEditEvent.eventName, (event) => {
            event.preventDefault();
            ...
            (new AjaxRequest(TYPO3.settings.ajaxUrls.dashboard_dashboard_edit))
                ...
                const data = await response.resolve();
                if (data.status === 'ok') {
                    const oldDashboard = this.dashboards.filter((dashboard) => {
        ...
        // Update dashboard
        this.addEventListener(DashboardUpdateEvent.eventName, (event) => {
            event.preventDefault();
            const { identifier, widgets, widgetPositions, } = event;
        ...
        // Delete dashboard
        this.addEventListener(DashboardDeleteEvent.eventName, (event) => {
            event.preventDefault();
            const { identifier } = event;
            (new AjaxRequest(TYPO3.settings.ajaxUrls.dashboard_dashboard_delete))
                ...
                const data = await response.resolve();
                if (data.status === 'ok') {
                    this.dashboards = this.dashboards.filter((dashboard) => {
                        return dashboard.identifier !== identifier;
                    });
    ...
    getDashboardByIdentifier(identifier) {
    ...
    async createDashboard() {
      ...
      <form>
        ...
        <div class="dashboard-modal-items">
          ${repeat(filteredPresets, (preset) => preset.identifier, (preset, index) => html `
        ...
        Modal.advanced({
            ...
            callback: (currentModal) => {
                currentModal.addEventListener('typo3-modal-shown', () => {
                    currentModal.querySelector('#dashboard-form-add-title')?.focus();
                });
                currentModal.querySelector('form').addEventListener('submit', (e) => {
                    e.preventDefault();
                    const form = e.target;
                    const formData = new FormData(form);
                    this.dispatchEvent(new DashboardAddEvent(formData.get('preset'), formData.get('title')));
    ...
    `;
        Modal.advanced({
            ...
            callback: (currentModal) => {
                currentModal.addEventListener('typo3-modal-shown', () => {
                    currentModal.querySelector('#dashboard-form-edit-title')?.focus();
                });
                currentModal.querySelector('form').addEventListener('submit', (e) => {
                    e.preventDefault();
                    const form = e.target;
                    const formData = new FormData(form);
                    this.dispatchEvent(new DashboardEditEvent(dashboard.identifier, formData.get('title')));
                    currentModal.hideModal();
                });
            },
            buttons: [
    ...
    deleteDashboard(dashboard) {
        const modal = Modal.confirm(lll('dashboard.delete'), lll('dashboard.delete.sure'), SeverityEnum.warning, [
            {
    ...
    async addWidget() {
        ...
        wizard.addEventListener(newRecordWizardEventName, async (event) => {
            ...
            if (data.status === 'ok') {
                ...
                if (item) {
                    ...
                    window.setTimeout(() => item.querySelector('.widget-actions > button:first-child')?.focus({ preventScroll: true, focusVisible: false }), 50);
                }
            }
            else {
                Notification.error('', data.message);
        ...
        Modal.advanced({
            ...
            content: wizard,
            callback: (currentModal) => {
                currentModal.addEventListener('button.clicked', () => {
                    currentModal.hideModal();
                });
            },
            buttons: [
      ...
      <div class="dashboard-header">
        ...
        <div class="dashboard-header-container">
          <div class="dashboard-tabs">
            ${repeat(this.dashboards, (dashboard) => dashboard.identifier, (dashboard) => html `
              <button
    ...
    renderContent() {
        if (this.currentDashboard) {
          ...
          >
            ${repeat(this.currentDashboard.widgetPositions[this.columns], (widget) => widget.identifier, (widget) => html `
              <div
                ...
                draggable="true"
                @pointerenter=${(e) => e.target.setAttribute('draggable', 'true')}
    ...
    getGridItemByIdentifier(identifier) {
    ...
    positionDraggingElement(event) {
        ...
        const gap = 20;
        const x = clamp(event.clientX - this.dragInformation.offsetX, containerRect.left - gap, containerRect.left + containerRect.width - itemRect.width + gap);
        const y = Math.max(containerRect.top - gap, event.clientY - this.dragInformation.offsetY);
    ...
    handleLegacyWidgetRefreshEvent(widget) {
    ...
    initializeCurrentDashboard() {
        ...
        const widgetSizeWidth = { small: 1, medium: 2, large: 4 };
        const widgetSizeHeight = { small: 1, medium: 2, large: 3 };
    ...
    widgetTryPlacementInNeighbourCells(item, occupiedCells, allowedDistance) {
    ...
    widgetPositionsArrange(items, previousArrangement, movedItem) {
        let occupiedCells = new Set();
        const placeInCurrentPosition = (item) => this.widgetPositionCanPlace(item, item.x, item.y, occupiedCells) ? { ...item } : null;
        const placeInNeighboursWithoutShiftingPreviousArrangements = (item) => previousArrangement === undefined ? null :
            this.widgetTryPlacementInNeighbourCells(item, 
            ...
            previousArrangement
                ...
                .reduce((set, item) => set.union(createSet(item)), new Set())
            ...
            movedItem);
        const placeInNeighbours = (item) => this.widgetTryPlacementInNeighbourCells(item, occupiedCells);
        const placeSomewhere = (item) => {
...
__decorate([
    state()
], Dashboard.prototype, "dashboards", void 0);
__decorate([
    state()
], Dashboard.prototype, "currentDashboard", void 0);
__decorate([
    state()
], Dashboard.prototype, "columns", void 0);
__decorate([
    state()
], Dashboard.prototype, "dragInformation", void 0);
__decorate([
    query('.dashboard-dragging-container')
], Dashboard.prototype, "draggingContainer", void 0);
Dashboard = __decorate([
    customElement('typo3-dashboard')
], Dashboard);
export { Dashboard };
let DashboardWidget = class DashboardWidget extends LitElement {
    constructor() {
        super(...arguments);
        this.moving = false;
        ...
        this.fetchTask = new Task(this, {
            ...
            onError: (error) => {
                console.error(`Error while retrieving widget [${this.identifier}]: ${error instanceof AjaxResponse ? `${error.response.status} ${error.response.statusText}` : error.message}`);
            },
        });
    }
    get widget() {
    ...
    `;
        const widgetLabel = (widget) => widget?.label || 'ERROR';
        const widgetContent = (widget) => widget
            ? unsafeHTML(widget.content)
            : html `<div class="widget-content-main">${lll('widget.error')}</div>`;
        const refreshButton = (loading = false) => html `
      <button
        type="button"
    ...
    `;
        const widgetRenderer = (widget, loading = false) => html `
      ...
      <div class="widget-content"
        @pointerenter=${(e) => e.target.closest('.dashboard-item').removeAttribute('draggable')}
        @pointerleave=${(e) => e.target.closest('.dashboard-item').setAttribute('draggable', 'true')}
    ...
    moveStart() {
        if (this.moving === false) {
            this.moving = true;
            this.dispatchEvent(new DashboardWidgetMoveIntendEvent(this.widget.identifier, DashboardWidgetMoveIntend.start));
        }
    }
    moveEnd() {
        if (this.moving === true) {
            this.moving = false;
            this.dispatchEvent(new DashboardWidgetMoveIntendEvent(this.widget.identifier, DashboardWidgetMoveIntend.end));
        }
    }
    handleMoveClick() {
        if (!this.moving) {
    ...
    handleMoveKeyDown(event) {
        ...
        let intend = DashboardWidgetMoveIntend.end;
        switch (event.code) {
            case 'Escape':
            case 'Enter':
            case 'Space':
        ...
        this.dispatchEvent(new DashboardWidgetMoveIntendEvent(this.widget.identifier, intend));
    }
    handleRefresh() {
        this.fetchTask.run();
    }
    handleRemove(event) {
        const modal = Modal.confirm(lll('widget.remove.confirm.title'), lll('widget.remove.confirm.message'), SeverityEnum.warning, [
            {
...
], DashboardWidget.prototype, "identifier", void 0);
__decorate([
    state()
], DashboardWidget.prototype, "moving", void 0);
