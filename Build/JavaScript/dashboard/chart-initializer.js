...
class ChartInitializer {
    ...
    initialize() {
        ...
        new RegularEvent(DashboardWidgetContentRenderedEvent.eventName, (e, htmlElement) => {
            e.preventDefault();
            const config = e.widget.eventdata;
            if (undefined === config || undefined === config.graphConfig) {
                return;
            }
            const _canvas = htmlElement.querySelector('canvas');
            let context;
            if (_canvas !== null) {
