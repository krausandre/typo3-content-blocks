...
class Router {
    constructor() {
        this.rootSelector = '.t3js-body';
        this.contentSelector = '.t3js-module-body';
        this.scaffoldSelector = '.t3js-scaffold';
        this.scaffoldContentOverlaySelector = '.t3js-scaffold-content-overlay';
        this.scaffoldMenuToggleSelector = '.t3js-topbar-button-modulemenu';
    }
    setContent(content) {
        const container = this.rootContainer.querySelector(this.contentSelector);
    ...
    initialize() {
        ...
        new RegularEvent('click', (event, target) => {
            ...
            if (isInline) {
                import(importModule).then(({ default: aModule }) => {
                    aModule.initialize(target);
                });
            }
            else {
                const modalTitle = target.closest('.card').querySelector('.card-title').innerHTML;
                const modalSize = target.dataset.modalSize || Modal.sizes.large;
                Modal.advanced({
    ...
    getUrl(action, controller, additionalQueryParams) {
    ...
    async handleAjaxError(error, outputContainer) {
        if (error.response?.status === 403) {
            ...
            if (this.context === 'backend') {
                this.rootContainer.replaceChildren(InfoBox.create(Severity.error, 'The Install Tool session expired. Please reload the backend and try again.'));
            }
            else {
            ...
            const message = '<div class="t3js-infobox callout callout-sm callout-danger">'
    ...
    login() {
        const outputContainer = document.querySelector('.t3js-login-output');
        outputContainer.innerHTML = '';
        (new AjaxRequest(this.getUrl()))
            .post({
            install: {
                action: 'login',
                token: document.querySelector('[data-login-token]').dataset.loginToken,
                password: document.querySelector('.t3-install-form-input-text').value,
            },
        })
            .then(async (response) => {
            const data = await response.resolve();
    ...
    registerScaffoldEvents() {
        ...
        document.querySelector(this.scaffoldMenuToggleSelector).addEventListener('click', (event) => {
            event.preventDefault();
            this.toggleMenu();
        });
        document.querySelector(this.scaffoldContentOverlaySelector).addEventListener('click', (event) => {
            event.preventDefault();
            this.toggleMenu(true);
        });
        document.querySelectorAll('[data-installroute-controller]').forEach((element) => {
