...
describe('@typo3/core/cache/dedupe-async-task', () => {
    it('dedupes', async () => {
        let counter = 0;
        const task = async () => {
            return ++counter;
        };
        const dedupe = new DedupeAsyncTask();
        const a = await dedupe.get('foo', task);
        const b = await dedupe.get('foo', task);
        expect(a).to.equal(1);
        expect(b).to.equal(1);
    });
    describe('aborts requests', () => {
        const boolTask = async (signal) => {
            await new Promise((resolve) => window.setTimeout(resolve, 0));
            return !signal.aborted;
        };
        it('if all requests are aborted', async () => {
            let wasAborted;
            ...
            const internalPromise = dedupe.promises.foo.promise;
        ...
        it('unless only first request is aborted', async () => {
            const dedupe = new DedupeAsyncTask();
            const abortControllerA = new AbortController();
        ...
        it('unless only last request is aborted', async () => {
            const dedupe = new DedupeAsyncTask();
            const abortControllerA = new AbortController();
            const aPromise = dedupe.get('foo', boolTask, abortControllerA.signal);
            let a;
        ...
        it('if previous request already succeeded', async () => {
            const dedupe = new DedupeAsyncTask();
