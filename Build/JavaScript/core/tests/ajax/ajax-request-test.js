...
describe('@typo3/core/ajax/ajax-request', () => {
    let promiseHelper;
    let fetchStub;
    beforeEach(() => {
    ...
    it('sends POST request with empty object', () => {
    ...
    describe('send GET requests', () => {
        ...
        for (const providedData of responseDataProvider()) {
            ...
            it('receives a ' + name + ' response', (done) => {
                ...
                (new AjaxRequest(new URL('https://example.com'))).get().then(async (response) => {
    ...
    describe('Aborts requests', () => {
        it('via abort() method', () => {
            ...
            request.get();
            request.abort();
            expect(fetchStub.firstCall.args[0].signal.aborted).to.be.true;
        });
        it('via signal option', () => {
