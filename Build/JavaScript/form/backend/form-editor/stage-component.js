...
function renderNestedSortableListItem(formElement) {
    ...
    try {
        ...
        assert(template.length > 0, 'No template found for element "' + formElement.get('__identifierPath') + '"', 1478987818);
    }
    template = $('<div></div>')
...
export function renderPagination() {
    ...
    $(getHelper().getDomElementDataIdentifierSelector('paginationTitle')).text(getFormElementDefinition(getRootFormElement(), 'paginationTitle')
...
export function getAllFormElementDomElements() {
    return $(getHelper().getDomElementDataAttribute('elementIdentifier', 'bracesWithKey'), stageDomElement);
}
/* *************************************************************
 * Abstract stage
 * ************************************************************/
...
export function renderFormDefinitionPageAsSortableList(pageIndex) {
    assert('number' === $.type(pageIndex), 'Invalid parameter "pageIndex"', 1478721208);
    return $('<ol></ol>')
...
export function removeAllStageToolbars() {
...
export function createAndAddAbstractViewFormElementToolbar(selectedFormElementDomElement, formElement, useFadeEffect) {
    if (getUtility().isUndefinedOrNull(formElement)) {
...
export function renderCheckboxTemplate(formElement, template) {
    ...
    eachTemplateProperty(formElement, template, function (propertyPath, propertyValue, domElement) {
        if (('boolean' === $.type(propertyValue) && propertyValue)
            || propertyValue === 'true'
            || propertyValue === 1
            || propertyValue === '1') {
...
export function renderSimpleTemplate(formElement, template) {
    ...
    Icons.getIcon(getFormElementDefinition(formElement, 'iconIdentifier'), Icons.sizes.small, null, Icons.states.default, Icons.markupIdentifiers.inline).then(function (icon) {
...
export function renderSimpleTemplateWithValidators(formElement, template) {
    ...
    renderSimpleTemplate(formElement, template);
    const validatorsTemplateContent = $(getHelper().getDomElementDataIdentifierSelector('validatorsContainer'), $(template)).clone();
...
export function renderSelectTemplates(formElement, template) {
    const multiValueTemplateContent = $(getHelper().getDomElementDataIdentifierSelector('multiValueContainer'), $(template)).clone();
    ...
    const appendMultiValue = (label, value, defaultValue) => {
        ...
        if (isPreselected) {
            getHelper().getTemplatePropertyDomElement('_label', rowTemplate).addClass(getHelper().getDomElementClassName('selected'));
        }
