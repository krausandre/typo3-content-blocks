...
import { PropertyGridEditorUpdateEvent } from '@typo3/form/backend/form-editor/component/property-grid-editor';
 ...
 */
function openTypo3WinBrowser(mode, params) {
    Modal.advanced({
...
function validateCollectionElement(propertyPath, editorHtml) {
    ...
    if (hasError) {
        getViewModel().setElementValidationErrorClass(getEditorControlsWrapperDomElement(editorHtml).closest(getHelper().getDomElementClassName('collectionElement', true)));
    }
    else {
        getViewModel().removeElementValidationErrorClass(getEditorControlsWrapperDomElement(editorHtml).closest(getHelper().getDomElementClassName('collectionElement', true)));
    }
}
/**
 * @throws 1489932939
...
function getFirstAvailableValidationErrorMessage(errorCodes, propertyData) {
    assert('array' === $.type(errorCodes), 'Invalid configuration "errorCodes"', 1489932939);
    assert('array' === $.type(propertyData), 'Invalid configuration "propertyData"', 1489932940);
...
function renewValidationErrorMessages(errorCodes, propertyData, value) {
    assert('array' === $.type(propertyData), 'Invalid configuration "propertyData"', 1489932942);
...
function setRandomIds(html) {
    assert('object' === $.type(html), 'Invalid input "html"', 1523904699);
    $(getHelper().getDomElementClassName('inspectorEditor', true)).each(function () {
        ...
        $(getHelper().getDomElementDataAttribute('randomId', 'bracesWithKey'), $parent).each(function () {
            const $element = $(this), targetAttribute = $element.attr(getHelper().getDomElementDataAttribute('randomIdTarget')), randomIdIndex = $element.attr(getHelper().getDomElementDataAttribute('randomIdIndex'));
...
export function getCollectionElementDomElement(collectionName, collectionElementIdentifier) {
    if (collectionName === 'finishers') {
        return $(getHelper().getDomElementDataAttribute('finisher', 'bracesWithKeyValue', [collectionElementIdentifier]), getFinishersContainerDomElement());
    }
    else {
        return $(getHelper().getDomElementDataAttribute('validator', 'bracesWithKeyValue', [collectionElementIdentifier]), getValidatorsContainerDomElement());
    }
}
...
export function renderCollectionElementEditors(collectionName, collectionElementIdentifier) {
    ...
    for (let i = 0; i < collectionElementEditorsLength; ++i) {
        ...
        const html = $(template.html());
        $(html).first()
            .addClass(getCollectionElementClass(collectionName, collectionElementConfiguration.editors[i].identifier))
            .addClass(getHelper().getDomElementClassName('inspectorEditor'));
...
export function renderCollectionElementSelectionEditor(collectionName, editorConfiguration, editorHtml) {
    ...
    assert('object' === $.type(editorConfiguration), 'Invalid parameter "editorConfiguration"', 1475423098);
    assert('object' === $.type(editorHtml), 'Invalid parameter "editorHtml"', 1475423099);
    assert(getUtility().isNonEmptyString(editorConfiguration.label), 'Invalid configuration "label"', 1475423100);
    assert('array' === $.type(editorConfiguration.selectOptions), 'Invalid configuration "selectOptions"', 1475423101);
    if (collectionName === 'finishers') {
        collectionContainer = getFinishersContainerDomElement();
...
export function renderCollectionElementHeaderEditor(editorConfiguration, editorHtml, collectionElementIdentifier, collectionName) {
    assert('object' === $.type(editorConfiguration), 'Invalid parameter "editorConfiguration"', 1475421258);
    assert(getUtility().isNonEmptyString(editorConfiguration.label), 'Invalid configuration "label"', 1475421257);
    assert('object' === $.type(editorHtml), 'Invalid parameter "editorHtml"', 1475421259);
    const setData = function (icon) {
        ...
        const editors = getFormEditorApp().getPropertyCollectionElementConfiguration(collectionElementIdentifier, collectionName).editors;
        if (!((editors.length === 2 && editors[0].identifier === 'header' && editors[1].identifier === 'removeButton') ||
...
export function renderTextEditor(editorConfiguration, editorHtml, collectionElementIdentifier, collectionName) {
    assert('object' === $.type(editorConfiguration), 'Invalid parameter "editorConfiguration"', 1475421053);
    assert('object' === $.type(editorHtml), 'Invalid parameter "editorHtml"', 1475421054);
...
export function renderValidationErrorMessageEditor(editorConfiguration, editorHtml) {
    assert('object' === $.type(editorConfiguration), 'Invalid parameter "editorConfiguration"', 1489874121);
    assert('object' === $.type(editorHtml), 'Invalid parameter "editorHtml"', 1489874122);
    ...
    getHelper().getTemplatePropertyDomElement('propertyPath', editorHtml).on('keyup paste', function () {
        ...
        getCurrentlySelectedFormElement().set(propertyPath, renewValidationErrorMessages(editorConfiguration.errorCodes, propertyData, $(this).val()));
    });
}
/**
 * @throws 1674826430
...
export function renderCountrySelectEditor(editorConfiguration, editorHtml, collectionElementIdentifier, collectionName) {
    assert('object' === $.type(editorConfiguration), 'Invalid parameter "editorConfiguration"', 1674826430);
    assert('object' === $.type(editorHtml), 'Invalid parameter "editorHtml"', 1674826431);
...
export function renderSingleSelectEditor(editorConfiguration, editorHtml, collectionElementIdentifier, collectionName) {
    assert('object' === $.type(editorConfiguration), 'Invalid parameter "editorConfiguration"', 1475421048);
    assert('object' === $.type(editorHtml), 'Invalid parameter "editorHtml"', 1475421049);
    ...
    assert('array' === $.type(editorConfiguration.selectOptions), 'Invalid configuration "selectOptions"', 1475421052);
    const propertyPath = getFormEditorApp().buildPropertyPath(editorConfiguration.propertyPath, collectionElementIdentifier, collectionName);
    getHelper()
        .getTemplatePropertyDomElement('label', editorHtml)
        .append(editorConfiguration.label);
...
export function renderMultiSelectEditor(editorConfiguration, editorHtml, collectionElementIdentifier, collectionName) {
    assert('object' === $.type(editorConfiguration), 'Invalid parameter "editorConfiguration"', 1485712399);
    assert('object' === $.type(editorHtml), 'Invalid parameter "editorHtml"', 1485712400);
    ...
    assert('array' === $.type(editorConfiguration.selectOptions), 'Invalid configuration "selectOptions"', 1485712403);
    const propertyPath = getFormEditorApp().buildPropertyPath(editorConfiguration.propertyPath, collectionElementIdentifier, collectionName);
    getHelper()
        .getTemplatePropertyDomElement('label', editorHtml)
        .append(editorConfiguration.label);
...
export function renderGridColumnViewPortConfigurationEditor(editorConfiguration, editorHtml) {
    assert('object' === $.type(editorConfiguration), 'Invalid parameter "editorConfiguration"', 1489528242);
    assert('object' === $.type(editorHtml), 'Invalid parameter "editorHtml"', 1489528243);
    assert(getUtility().isNonEmptyString(editorConfiguration.label), 'Invalid configuration "label"', 1489528244);
    assert('array' === $.type(editorConfiguration.configurationOptions.viewPorts), 'Invalid configurationOptions "viewPorts"', 1489528245);
    ...
    const initNumbersOfColumnsField = function (element) {
        ...
        getHelper()
            ...
            .append(editorConfiguration.configurationOptions.numbersOfColumnsToUse.label
...
export function renderPropertyGridEditor(editorConfiguration, editorHtml, collectionElementIdentifier, collectionName) {
    assert('object' === $.type(editorConfiguration), 'Invalid parameter "editorConfiguration"', 1475419226);
    assert('object' === $.type(editorHtml), 'Invalid parameter "editorHtml"', 1475419227);
    assert('boolean' === $.type(editorConfiguration.enableAddRow), 'Invalid configuration "enableAddRow"', 1475419228);
    assert('boolean' === $.type(editorConfiguration.enableDeleteRow), 'Invalid configuration "enableDeleteRow"', 1475419230);
    assert('boolean' === $.type(editorConfiguration.isSortable), 'Invalid configuration "isSortable"', 1475419229);
...
export function renderRequiredValidatorEditor(editorConfiguration, editorHtml, collectionElementIdentifier, collectionName) {
    assert('object' === $.type(editorConfiguration), 'Invalid parameter "editorConfiguration"', 1475417093);
    assert('object' === $.type(editorHtml), 'Invalid parameter "editorHtml"', 1475417094);
    ...
    const showValidationErrorMessage = function () {
        ...
        getHelper().getTemplatePropertyDomElement('validationErrorMessage-propertyPath', validationErrorMessageTemplateClone).on('keyup paste', function () {
            ...
            getCurrentlySelectedFormElement().set(validationErrorMessagePropertyPath, renewValidationErrorMessages(editorConfiguration.configurationOptions.validationErrorMessage.errorCodes, propertyData, $(this).val()));
        });
    };
...
export function renderCheckboxEditor(editorConfiguration, editorHtml, collectionElementIdentifier, collectionName) {
    assert('object' === $.type(editorConfiguration), 'Invalid parameter "editorConfiguration"', 1476218671);
    assert('object' === $.type(editorHtml), 'Invalid parameter "editorHtml"', 1476218672);
    ...
    const propertyPath = getFormEditorApp()
        .buildPropertyPath(editorConfiguration.propertyPath, collectionElementIdentifier, collectionName);
    const propertyData = getCurrentlySelectedFormElement().get(propertyPath);
    if (('boolean' === $.type(propertyData) && propertyData)
        || propertyData === 'true'
        || propertyData === 1
        || propertyData === '1') {
        $('input[type="checkbox"]', $(editorHtml)).prop('checked', true);
...
export function renderTextareaEditor(editorConfiguration, editorHtml, collectionElementIdentifier, collectionName) {
    assert('object' === $.type(editorConfiguration), 'Invalid parameter "editorConfiguration"', 1475412567);
    assert('object' === $.type(editorHtml), 'Invalid parameter "editorHtml"', 1475412568);
...
export function renderTypo3WinBrowserEditor(editorConfiguration, editorHtml, collectionElementIdentifier, collectionName) {
    assert('object' === $.type(editorConfiguration), 'Invalid parameter "editorConfiguration"', 1477300587);
    assert('object' === $.type(editorHtml), 'Invalid parameter "editorHtml"', 1477300588);
...
export function renderRemoveElementEditor(editorConfiguration, editorHtml, collectionElementIdentifier, collectionName) {
    ...
    if (getUtility().isUndefinedOrNull(collectionElementIdentifier)) {
        $('button', $(editorHtml))
            .addClass(getHelper().getDomElementClassName('buttonFormElementRemove') + ' ' +
            getHelper().getDomElementClassName('buttonFormEditor'));
    }
    else {
        $('button', $(editorHtml)).addClass(getHelper().getDomElementClassName('buttonCollectionElementRemove'));
    }
...
export function renderFormElementSelectorEditorAddition(editorConfiguration, editorHtml, propertyPath) {
    assert('object' === $.type(editorConfiguration), 'Invalid parameter "editorConfiguration"', 1484574704);
    assert('object' === $.type(editorHtml), 'Invalid parameter "editorHtml"', 1484574705);
    assert(getUtility().isNonEmptyString(propertyPath), 'Invalid parameter "propertyPath"', 1484574706);
    const formElementSelectorControlsWrapper = $(getHelper().getDomElementDataIdentifierSelector('formElementSelectorControlsWrapper'), editorHtml);
    if (editorConfiguration.enableFormelementSelectionButton === true) {
        if (formElementSelectorControlsWrapper.length === 0) {
            return;
        }
        const formElementSelectorSplitButtonListContainer = $(getHelper().getDomElementDataIdentifierSelector('formElementSelectorSplitButtonListContainer'), editorHtml);
        ...
        if (nonCompositeNonToplevelFormElements.length === 0) {
            Icons.getIcon(getHelper().getDomElementDataAttributeValue('iconNotAvailable'), Icons.sizes.small, null, Icons.states.default).then(function (icon) {
            ...
            $.each(nonCompositeNonToplevelFormElements, function (i, nonCompositeNonToplevelFormElement) {
                Icons.getIcon(getFormElementDefinition(nonCompositeNonToplevelFormElement, 'iconIdentifier'), Icons.sizes.small, null, Icons.states.default).then(function (icon) {
                    const itemTemplate = $('<li data-no-sorting>'
