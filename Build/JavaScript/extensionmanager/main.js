...
class ExtensionManager {
    constructor() {
        this.searchFilterSessionKey = 'tx-extensionmanager-local-filter';
        DocumentService.ready().then(() => {
            ...
            if (extensionList !== null) {
                ...
                new RegularEvent('click', (e, target) => {
                    e.preventDefault();
                    Modal.confirm(TYPO3.lang['extensionList.databaseReload.title'], TYPO3.lang['extensionList.databaseReload.message'], Severity.warning, [
                        {
                            text: TYPO3.lang['button.cancel'],
                            active: true,
                            btnClass: 'btn-default',
                            ...
                            trigger: () => {
                                NProgress.start();
                                new AjaxRequest(target.href).post({}).then(() => {
                                    location.reload();
                                }).finally(() => {
                                    NProgress.done();
                                    Modal.dismiss();
    ...
    removeExtensionFromDisk(trigger) {
        NProgress.start();
        new AjaxRequest(trigger.href).post({}).then(() => {
            location.reload();
        }).finally(() => {
            NProgress.done();
        });
    ...
    async updateExtension(response) {
        ...
        container.append(updateConfirmationTitle, updateConfirmationMessage, form);
        NProgress.done();
        Modal.confirm(TYPO3.lang['extensionList.updateConfirmation.questionVersionComments'], container, Severity.warning, [
            {
                ...
                trigger: (e, modal) => {
                    ...
                    new AjaxRequest(data.url).post({
                        version: modal.querySelector('input[name="version"]:checked')?.value,
                    }).finally(() => {
                        location.reload();
                    });
                    modal.hideModal();
