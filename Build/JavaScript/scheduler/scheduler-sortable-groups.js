...
class SchedulerSortableGroups {
    constructor() {
        this.container = '.t3js-group-draggable-container';
        this.dragHandle = '.t3js-group-draggable-handle';
        this.initialize();
    }
    initialize() {
        ...
        if (element) {
            new Sortable(element, {
                ...
                onSort: (event) => {
                    ...
                    let letMoveTarget = 0;
                    if (previousItem) {
                        const previousUid = previousItem.dataset.taskGroupId;
                        letMoveTarget = Number('-' + previousUid);
                    }
                    const uid = Number(event.item.dataset.taskGroupId);
                    const table = 'tx_scheduler_task_group';
            ...
            document.querySelectorAll(this.dragHandle).forEach(handle => {
