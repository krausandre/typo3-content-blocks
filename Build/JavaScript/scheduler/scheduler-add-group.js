...
class SchedulerAddGroups {
    constructor() {
        this.selector = '.t3js-create-group';
        this.initialize();
    }
    initialize() {
        ...
        `;
                    ...
                    buttons: [
                        {
                            ...
                            trigger: () => {
                                const form = Modal.currentModal.querySelector('form[name="scheduler-create-group"]');
                                form.requestSubmit();
                            },
                ...
                modal.addEventListener('typo3-modal-shown', () => {
                    const input = Modal.currentModal.querySelector('input[name="action[createGroup]"]');
                    input.focus();
                });
            });
        }
    ...
    createGroup(e) {
        ...
        return (new AjaxRequest(TYPO3.settings.ajaxUrls.record_process)).post(params, {
            ...
            const select = document.querySelector('#task_group');
            if (select) {
                // This is an ugly hack to pre-select the created group.
                const form = document.forms[0];
                const selectLatestGroup = form.querySelector('[name="tx_scheduler[select_latest_group]"]');
